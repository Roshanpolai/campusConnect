...
import { Search, Bell, Moon, Sun, Menu, ChevronDown, LogOut, UserCircle, Settings } from "lucide-react";
import { useAuth } from "../../context/AuthContext.jsx";
import api from "../../api/axios.js";

export default function Navbar({ onMenuClick, searchPlaceholder = "Search anything..." }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-surface-border bg-surface/80 backdrop-blur px-4 py-3 sm:px-6">
      ...
      <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
        <button onClick={() => setDark((d) => !d)} className="btn-ghost !p-2.5" aria-label="Toggle dark mode">
          {dark ? <Sun size={18} /> : <Moon size={18} />}
        </button>

        <div className="relative" ref={notifRef}>
          <button onClick={() => setNotifOpen((o) => !o)} className="btn-ghost relative !p-2.5" aria-label="Notifications">
            <Bell size={18} />
            {unreadCount > 0 && (
          ...
          {notifOpen && (
            <div className="card absolute right-0 mt-2 w-80 p-2 animate-fade-in">
              <p className="px-2 py-1.5 text-sm font-semibold text-ink-900">Notifications</p>
              <div className="max-h-80 overflow-y-auto">
                ...
                {notifications.slice(0, 8).map((n) => (
                  ...
                  >
                    <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${!n.read ? "bg-primary-500" : "bg-transparent"}`} />
                    <span>
        ...
        <div className="relative" ref={profileRef}>
          <button onClick={() => setProfileOpen((o) => !o)} className="flex items-center gap-2 rounded-xl py-1 pl-1 pr-2 hover:bg-surface">
            <img
          ...
          {profileOpen && (
            <div className="card absolute right-0 mt-2 w-56 p-2 animate-fade-in">
              ...
              <Link to="/profile" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-ink-700 hover:bg-surface">
                <UserCircle size={16} /> Profile
              </Link>
              <Link to="/settings" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-ink-700 hover:bg-surface">
                <Settings size={16} /> Settings
              </Link>
              <button onClick={logout} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm text-rose-600 hover:bg-rose-50">
